...
impl ToneSandhi {
    ...
    fn neural_sandhi(
        ...
        if let Some(last_char) = word.chars().last() {
            if "吧呢哈啊呐噻嘛吖嗨呐哦哒额滴哩哟喽啰耶喔诶".contains(last_char)
                || "的地得".contains(last_char)
        ...
        if MUST_NEURAL_TONE_WORDS.contains(word)
            ...
            || ("了着过".contains(word) && ["ul", "uz", "ug"].contains(&pos))
            || ("们子".contains(word.chars().last().unwrap_or_default())
                && ["r", "n"].contains(&pos)
                && !MUST_NOT_NEURAL_TONE_WORDS.contains(word))
            || ("上下里".contains(word.chars().last().unwrap_or_default())
                && ["s", "l", "f"].contains(&pos))
        {
            let f_pre = finals[finals_len - 1]
                .chars()
